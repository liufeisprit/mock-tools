...
const SideModal = ({
  ...
  const updateCacheData = ({
    ...
    const newData = requestList.map((v, i) => {
      if (i == index) {
        const _ = v
        _.mockUrl = mockUrl || "";
        _.mockMethod = mockMethod || "";
        _.isOpen = !!isOpen;
        // 从列表修改默认设置值
        _.activeType = activeType || (mockUrl ? 2 : 1);
        _.data = data;
        return _
      }
      return v
  ...
  const handleSwitchChange =  async (
    ...
    updateCacheData({ index, mockUrl, isOpen, activeType, data });
  };
  ...
  const urlMockOnChange = async (
    ...
    try {
      // response
      updateCacheData({ index, mockUrl, mockMethod, isOpen, activeType, data });
    } catch (error) {
  ...
  return (
    <Drawer
      ...
      extra={
        <>
          <Button type="link" onClick={onHide}>隐藏工具</Button>
          <Radio.Group
