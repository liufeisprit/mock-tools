...
const mockInterface: Record<string, MockInterfaceItem> = {}
const mockRequestEffect = {}
const requestListSendData: Record<string, ReportListDataItem> = {}
const isRealRequest = new WeakMap()
// mock-sdk是否加载完成
let isMockSdkLoad = false
// 发送数据到xhr-mocker-panel-sdk
const sendResponse = (params: {
  requestListData: Record<string, ReportListDataItem>,
  rules?: string[],
  excludeRules?: string[],
  sdkLoaded?: boolean
}) => {
...
/**
 * @name sdk 主函数，用于开启拦截替换 XHR 和 fetch 请求，发送接口数据到 web 面板
...
const startMock = async(options) => {
  ...
  // 上报数据
  let requestListData: Record<string, ReportListDataItem> = {}
  ...
  XMLHttpRequest.prototype.open = function(method, url: string, ...args) {
    ...
    if (isMock && mockInterface[url_]) {
      ...
      if (mockInterface[url_].activeType == 2 && mockInterface[url_].mockUrl) {
        ...
        if (mockInterface[url_].mockMethod) {
          curArgs[0] = mockInterface[url_].mockMethod
        }
        printLog({
          url: url_,
          mockUrl: mockInterface[url_].mockUrl,
          method: mockInterface[url_].mockMethod || method
        })
      }
  ...
  window.fetch = function(url, options) {
    ...
    const _this = this
    const isMock = mockInterface[url_] && mockInterface[url_].isOpen
    const isRequestMock = isMock && mockInterface[url_].activeType == 2
      ...
      if (isRequestMock) {
        ...
        if (typeof curArgs[0] === 'string') {
          ...
          if(mockInterface[url_].mockMethod) {
            curArgs[1] = {
              ...curArgs[1],
              method: mockInterface[url_].mockMethod
            }
          }
          ...
          const newRequest = new Request('//' + mockInterface[url_].mockUrl, {
            method: mockInterface[url_].mockMethod || curArgs[0].method,
        ...
        printLog({
          url: url_,
          mockUrl: mockInterface[url_].mockUrl,
          method: mockInterface[url_].mockMethod || options?.method
        })
      }
      sendRequestEffect(url_, isMockSdkLoad)
    }
